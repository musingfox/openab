...
impl AdapterRouter {
    ...
    pub async fn stream_prompt_blocks(
        ...
        // An adapter's explicit preferred_table_mode() (e.g. Zulip → Off to
        // preserve #**stream>topic** / @**user** syntax) takes precedence.
        let table_mode = adapter.preferred_table_mode().unwrap_or(
            if adapter.renders_native_tables() {
                TableMode::Off
            } else {
                self.table_mode
            },
...
mod directive_tests {
    ...
    fn zulip_outbound_preserves_stream_topic_link() {
        let input = "[[resolve]] See #**general>deploy** for context.";
        let (directives, content) = parse_output_directives(input);
        assert!(directives.resolve, "resolve directive must be set");
        assert!(
            content.contains("#**general>deploy**"),
            "stream-topic link must survive directive strip, got: {content:?}"
        );
    }
